...
LogFile = os.getcwd() + "\\Log\\" + config['WorkConfig']['LogName'] + cur_date.strftime("%d.%m.%Y") + ".log"
        ...
        while row:
            ...
            if phone_id in stoplist:
                ...
                if checktaksofonotzvon(row[0]):
                    ...
                    slot = row[23]
                    logging.info("      код формирования отзвона")
                    ...
                    with open(LogWork, "a") as text_file:
                        text_file.write("Таксофон id : %4s, [%s %s]\n" % (row[0], s_cur_date, Reg_Date_Time_s))
...
con_ib.close()
con_stat.close()
logging.info("время работы скрипта - %s (без учата задержки после запуска)" % (datetime.datetime.now() - startTime))
logging.info("~" * 80)